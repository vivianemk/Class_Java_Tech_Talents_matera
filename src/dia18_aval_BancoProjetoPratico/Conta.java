...
// 	metodo para adicionar historico(extrato)
	...
	public void realizarTransferencia(Conta conta, double valor) {
		...
		if(this.contaAtiva) {
			if (valor<=this.saldo) {
				...
				this.movimentacoes.add(-valor);
				System.out.printf("Transferência no valor %f realizado com sucesso\n", valor);
			}else {
				System.out.println("Transferência não permitido, não há valor disponível na conta");
			}
		}
